function removeAccents(str) {
  ...
  return str;
    ...
    .then(data => {
       ...
       <div
       class="flex flex-col justify-center items-center gap-3 bg-[#FBF7F2] divTable" data-index="${item.id}">
      ...
      <div class="w-full px-[10px] py-[20px]">
       ...
       searchTable.addEventListener('keyup',(e)=>{
          const newArr = data.filter(item => removeAccents(item.title.toLowerCase()).includes(removeAccents(e.target.value.toLowerCase())));
          if(newArr.length > 0){
            const div = document.getElementById("tables");
            div.innerHTML = newArr.map(item => `
            <div
            class="flex flex-col justify-center items-center gap-3 bg-[#FBF7F2] divTable" data-index="${item.id}">
            <div class="w-full h-full rounded-tr-2xl rounded-tl-2xl">
              <img
